 ...
 * rather than integers. FPGrowth is described here:
 * <br/><br/>
 *
...
public class AlgoFPGrowth_Strings {
    ...
    public void runAlgorithm(String input, String output, double minsupp) throws FileNotFoundException, IOException {
        ...
        File itemsupport = new File("1-itemset.txt");
        File frequentitemsupport = new File("frequent1-itemset.txt");
        BufferedWriter frequentitemsupportwriter = new BufferedWriter(new FileWriter(frequentitemsupport));
        BufferedWriter itemsetwriter = new BufferedWriter(new FileWriter(itemsupport));
        mapSupport.entrySet().forEach((entry) -> {
            try {
                double support = (double) entry.getValue() / transactionCount;
                itemsetwriter.write(entry.getKey() + ",");
                itemsetwriter.write(entry.getValue() + ",");
                itemsetwriter.write(String.format("%.2f", support));
                itemsetwriter.newLine();
                
                if(support >= minsupp){
                    frequentitemsupportwriter.write(entry.getKey()+ ",");
                    frequentitemsupportwriter.write(entry.getValue() + ",");
                    frequentitemsupportwriter.write(String.format("%.2f", support));
                    frequentitemsupportwriter.newLine();
                }
            } catch (IOException ex) {
                Logger.getLogger(AlgoFPGrowth_Strings.class.getName()).log(Level.SEVERE, null, ex);
            }
        });
        itemsetwriter.close();
        frequentitemsupportwriter.close();
...
//	}
    ...
    private void writeItemsetToFile(String[] itemset, int support) throws IOException {
        ...
        * */
        ...
        // append the support of the itemset
        buffer.append(':');
        buffer.append(String.format("%.2f", (double) support/ transactionCount));
        // write the strinbuffer and create a newline so that we are
        // ready for the next itemset to be written
        writer.write(buffer.toString());
        writer.newLine();
